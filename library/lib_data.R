...
convert.to.shp <- function(x, lat.name="lat", lon.name="lon", measure.vars=setdiff(names(data), c(lat.name, lon.name))) {
  #
  # Faster, vectorised implementation of shapefiles::convert.to.shapefile for data.frame containing values at points
  #
  # x             data.frame with one line per point and columns with latitude, longitude and variables of interest
  # lat/lon.name  names of the latitude and longitude columns
  # measure.vars  names of the variables to include as attributes (by default, all but lat and lon). Names can be abbreviated and will be expanded if needed.
  #

  suppressPackageStartupMessages(require("plyr", quietly=TRUE))

  # Check input arguments
  if (lat.name %in% names(x)) {
    names(x)[which(names(x)==lat.name)] <- "Y"
  } else {
    stop("lat.name : \"", lat.name, "\" is not a column of ", deparse(substitute(x)))
  }
  if (lon.name %in% names(x)) {
    names(x)[which(names(x)==lon.name)] <- "X"
  } else {
    stop("lon.name : \"", lon.name, "\" is not a column of ", deparse(substitute(x)))
  }
  measure.vars <- match.vars(measure.vars, setdiff(names(x), c("lon", "lat")), quiet=TRUE)

  # Create the shapefile structure
  # shp : A location data.frame + a specific header
  #       The location data.frame as columns : unique id, X, Y, and shape type
  # shx : A matrix with "Offset" and "Length" of each feature (i.e., each point) + a specific header
  # dbf : Attributes table + no header.
  #       The attributes table is a data.frame with the unique id for each point and associated values


  # generate a unique record number for each point
  n <- nrow(x)
  x$record <- 1:n

  # vector with the "content length" of each record
  # no idea what this is...
  v.content.length <- rep(10, times=n)

  # shp
  shpTable <- x[,c("record", "X", "Y")]
  shpTable$shape.type <- 1
  shpTable[, c("X", "Y")] <- round(shpTable[, c("X", "Y")], 5)
  ...
  shp <- list(shp=shpTable, header=shpHeader)

  # shx
  shxTable <- cbind(
    Offset = c(50, (cumsum(v.content.length + 4) + 50)[-n]),
    Length = v.content.length
  )

  shxHeader <- shpHeader
  shxHeader$file.length <- 50 + 4 * n

  shx <- list(index=shxTable, header=shxHeader)

  # dbf
  dbfTable <- x[,c("record", measure.vars)]

  dbf <- list(dbf=dbfTable, header=NULL)

  return(list(shp=shp, shx=shx, dbf=dbf))
}
